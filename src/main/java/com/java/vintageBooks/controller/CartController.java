...
public class CartController {
    ...
    @GetMapping("/buyerscartshow")
    public String showInCart(HttpSession session,Model model){
        Buyer buyer=(Buyer) session.getAttribute("buyerobject");
        List<BuyerCart> buyerCart=buyerCartService.getCartByUsername(buyer.getUsername());
        model.addAttribute("buyerCartList",buyerCart);
        model.addAttribute("count",buyerCart.size());
        return "buyercart";
    }
}
